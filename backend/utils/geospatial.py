...
class GeospatialProcessor:
    ...
    def is_in_cundinamarca(self, lat: float, lon: float) -> bool:
        ...
        try:
            return (
                self.cundinamarca_bounds["min_lat"]
                <= lat
                <= self.cundinamarca_bounds["max_lat"]
                and self.cundinamarca_bounds["min_lon"]
                <= lon
                <= self.cundinamarca_bounds["max_lon"]
            )
    ...
    def validate_coordinates(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.is_in_cundinamarca(lat, lon):
                ...
                validation_result["location_info"][
                    "nearest_municipio"
                ] = nearest_municipio
                ...
                if distance > 50:  # Más de 50 km del municipio más cercano
                    validation_result["warnings"].append(
                        f"Coordenadas muy alejadas del municipio más cercano ({distance:.1f} km)"
                    )
            else:
                validation_result["warnings"].append(
                    "Coordenadas fuera de Cundinamarca"
                )
    ...
    def process_station_locations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            result_df["en_cundinamarca"] = result_df.apply(
                lambda row: self.is_in_cundinamarca(row["latitud"], row["longitud"]),
                axis=1,
            )

            # Encontrar municipio más cercano para cada estación
            municipio_info = result_df.apply(
                lambda row: self.find_nearest_municipio(
            ...
            result_df["distancia_bogota"] = result_df.apply(
                lambda row: self.calculate_distance(
                    row["latitud"], row["longitud"], bogota_lat, bogota_lon
                ),
                axis=1,
            )
    ...
    def filter_by_region(
        self, df: pd.DataFrame, region: str = "cundinamarca"
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if region.lower() == "all":
                ...
                mask = df.apply(
                    lambda row: self.is_in_cundinamarca(
                        row["latitud"], row["longitud"]
                    ),
                    axis=1,
                )
                ...
                logger.warning(
                    f"Región '{region}' no reconocida, retornando todos los datos"
                )
                return df
    ...
    def calculate_spatial_statistics(
        ...
        try:
            ...
            if "municipio_mas_cercano" in df.columns:
                spatial_stats = (
                    df.groupby("municipio_mas_cercano")[variable]
                ...
                spatial_stats = (
                    df_temp.groupby(["lat_rounded", "lon_rounded"])[variable]
                    .agg(["count", "mean", "std", "min", "max"])
                    .to_dict("index")
                )
            ...
            if len(df) > 2:
                ...
                spatial_stats["gradient"] = {
                    "latitud_correlation": float(lat_corr)
                    if not pd.isna(lat_corr)
                    else 0.0,
                    "longitud_correlation": float(lon_corr)
                    if not pd.isna(lon_corr)
                    else 0.0,
                }
