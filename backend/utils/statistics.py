...
class StatisticalAnalyzer:
    ...
    def calculate_descriptive_statistics(self, df: pd.DataFrame, 
                ...
                if var in df.columns:
                    ...
                    if len(data) > 0:
                        stats_dict[var] = {
                            ...
                            'missing_percentage': float((df[var].isnull().sum() / len(df)) * 100)
                        }
    ...
    def perform_normality_test(self, df: pd.DataFrame, variables: List[str] = None) -> Dict[str, Any]:
        """
                        ...
                        if len(common_idx) > 3:
                            try:
                                if method == 'pearson':
                                    ...
                                    _, p_val = kendalltau(data1[common_idx], data2[common_idx])
    ...
    def perform_trend_analysis(self, df: pd.DataFrame, variable: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'fecha' not in df.columns:
                return {'error': 'Columna de fecha requerida para análisis de tendencias'}
    ...
    def _analyze_seasonality(self, df: pd.DataFrame, variable: str) -> Dict[str, Any]:
        ...
        try:
            ...
            monthly_stats = df_temp.groupby('month')[variable].agg([
    ...
    def detect_outliers(self, df: pd.DataFrame, variables: List[str] = None,
    ...
    def perform_station_comparison(self, df: pd.DataFrame, variable: str) -> Dict[str, Any]:
        """
        ...
        try:
            if variable not in df.columns or 'estacion' not in df.columns:
                raise ValueError("Se requieren columnas 'estacion' y la variable especificada")
            
            # Estadísticas por estación
            station_stats = df.groupby('estacion')[variable].agg([
            ...
            if len(stations) > 1:
                station_groups = [df[df['estacion'] == station][variable].dropna() 
    ...
    def generate_statistical_report(self, df: pd.DataFrame, 
        ...
        try:
            ...
            for var, stats in desc_stats.items():
                ...
                report_lines.append(f"  - Datos faltantes: {stats['missing_percentage']:.1f}%")
            ...
            for var, results in normality_results.items():
                ...
                if 'shapiro_wilk' in results:
                    ...
                    if sw['p_value'] is not None:
                        report_lines.append(f"  - Shapiro-Wilk: p = {sw['p_value']:.4f}")
                        report_lines.append(f"  - Normal: {'Sí' if sw['is_normal'] else 'No'}")
