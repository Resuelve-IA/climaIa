...
class DataValidator:
    ...
    def _validate_data_structure(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        climate_variables = ['temperatura', 'precipitacion', 'humedad', 'presion', 'viento']
        has_climate_data = any(any(var in col.lower() for var in climate_variables) 
        ...
        for col, (min_val, max_val) in self.valid_ranges.items():
            if col in df.columns:
                ...
                if len(out_of_range) > 0:
                    ...
                    if len(out_of_range) > len(df) * 0.1:  # Más del 10% de valores fuera de rango
        ...
        try:
            ...
            if 'estacion' in df.columns:
                ...
                if len(duplicate_dates) > 0:
                    warnings.append(f"Se encontraron fechas duplicadas en {len(duplicate_dates)} combinaciones estación-fecha")
            
            # Verificar gaps temporales grandes
            if 'estacion' in df.columns:
                for station in df_temp['estacion'].unique():
                    station_data = df_temp[df_temp['estacion'] == station].sort_values('fecha')
                    
                    if len(station_data) > 1:
                        date_diffs = station_data['fecha'].diff().dt.days
                        large_gaps = date_diffs[date_diffs > 30]  # Gaps mayores a 30 días
                        
                        if len(large_gaps) > 0:
                            warnings.append(f"Estación {station}: {len(large_gaps)} gaps temporales mayores a 30 días")
    ...
    def _validate_logical_consistency(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        if all(col in df.columns for col in temp_cols):
            ...
            if len(invalid_temp) > 0:
                ...
                if len(invalid_temp) > len(df) * 0.05:  # Más del 5% de registros inconsistentes
                    errors.append(error_msg)
                else:
                    warnings.append(error_msg)
        ...
        if 'precipitacion' in df.columns:
            ...
            if len(negative_precip) > 0:
                errors.append(f"Se encontraron {len(negative_precip)} registros con precipitación negativa")
        
        # Verificar que humedad está entre 0 y 100
        if 'humedad_relativa' in df.columns:
            invalid_humidity = df[(df['humedad_relativa'] < 0) | (df['humedad_relativa'] > 100)]
            if len(invalid_humidity) > 0:
                errors.append(f"Se encontraron {len(invalid_humidity)} registros con humedad fuera del rango [0, 100]")
    ...
    def _calculate_quality_score(self, df: pd.DataFrame) -> float:
        ...
        score -= missing_percentage * 0.5  # Penalizar 0.5 puntos por cada % de datos faltantes
        ...
        out_of_range_percentage = (out_of_range_count / (len(df) * len(df.columns))) * 100
        score -= out_of_range_percentage * 1.0  # Penalizar 1 punto por cada % de valores fuera de rango
        ...
        if all(col in df.columns for col in ['temperatura_minima', 'temperatura_promedio', 'temperatura_maxima']):
            ...
            score -= invalid_percentage * 2.0  # Penalizar 2 puntos por cada % de inconsistencias
        ...
        try:
            ...
            if 'latitud' in station_data and 'longitud' in station_data:
                try:
                    ...
                    if not (-90 <= lat <= 90):
                        validation_results['errors'].append("Latitud fuera del rango válido [-90, 90]")
                    
                    if not (-180 <= lon <= 180):
                        validation_results['errors'].append("Longitud fuera del rango válido [-180, 180]")
                    
                    validation_results['validation_summary']['coordenadas'] = {'latitud': lat, 'longitud': lon}
                    
                except (ValueError, TypeError):
                    validation_results['errors'].append("Coordenadas no son valores numéricos válidos")
            ...
            if 'elevacion' in station_data:
                try:
                    ...
                    if elev < -1000 or elev > 10000:  # Rango razonable para Colombia
                        validation_results['warnings'].append("Elevación fuera del rango típico para Colombia")
                    
                    validation_results['validation_summary']['elevacion'] = elev
                    
                except (ValueError, TypeError):
                    validation_results['errors'].append("Elevación no es un valor numérico válido")
    ...
    def get_validation_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        report.append(f"- Score de calidad: {validation_results.get('data_quality_score', 0):.2f}%")
