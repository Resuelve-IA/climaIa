...
async def extract_climate_data(request: ClimateDataRequest):
    ...
    """
    try:
        logger.info(f"Extrayendo datos para período: {request.start_date} - {request.end_date}")
...
async def process_climate_data(file_path: str = Query(..., description="Ruta del archivo CSV con datos")):
    """
...
async def create_visualizations(request: VisualizationRequest):
    ...
    try:
        ...
        if 'temperatura' in request.plot_types or 'all' in request.plot_types:
            temp_plots = visualization_service.create_temperature_analysis_plots(df, output_dir)
            visualizations.update(temp_plots)
        
        if 'precipitacion' in request.plot_types or 'all' in request.plot_types:
            precip_plots = visualization_service.create_precipitation_analysis_plots(df, output_dir)
        ...
        if 'station_comparison' in request.plot_types or 'all' in request.plot_types:
            station_plot = visualization_service.create_station_comparison_plot(df, output_dir)
            visualizations['station_comparison'] = station_plot
        
        if 'anomalies' in request.plot_types or 'all' in request.plot_types:
            anomaly_plot = visualization_service.create_anomaly_detection_plot(df, output_dir)
        ...
        raise HTTPException(status_code=500, detail=f"Error creando visualizaciones: {str(e)}")
...
async def get_stations():
    ...
    try:
        ...
        for station in stations:
            ...
            processed_stations.append(StationDataResponse(
...
async def get_station(station_code: str):
    ...
    try:
        ...
        if not station:
            raise HTTPException(status_code=404, detail=f"Estación {station_code} no encontrada")
        ...
        raise HTTPException(status_code=500, detail=f"Error obteniendo estación: {str(e)}")


@router.post("/validate-data", response_model=ValidationResponse)
async def validate_data(file_path: str = Query(..., description="Ruta del archivo CSV con datos")):
    """
...
async def spatial_analysis(
    ...
    try:
        ...
        if 'latitud' not in df.columns or 'longitud' not in df.columns:
            raise HTTPException(status_code=400, detail="Datos de coordenadas no disponibles")
        ...
        return {
            ...
            "stations_in_cundinamarca": spatial_summary.get('stations_in_cundinamarca', 0),
            "total_stations": spatial_summary.get('total_stations', 0)
        }
        
    except Exception as e:
        logger.error(f"Error en análisis espacial: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error en análisis espacial: {str(e)}")
