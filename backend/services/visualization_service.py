...
class VisualizationService:
    ...
    def create_temperature_analysis_plots(self, df: pd.DataFrame, output_dir: str) -> Dict[str, str]:
        """
        ...
        try:
            ...
            if temp_cols:
                fig = make_subplots(rows=1, cols=len(temp_cols), subplot_titles=temp_cols)
    ...
    def create_precipitation_analysis_plots(self, df: pd.DataFrame, output_dir: str) -> Dict[str, str]:
        """
        ...
        try:
            ...
            if 'fecha' in df.columns and 'precipitacion' in df.columns:
                ...
                monthly_precip = df_temp.groupby(['año', 'mes'])['precipitacion'].sum().reset_index()
    ...
    def create_anomaly_detection_plot(self, df: pd.DataFrame, output_dir: str) -> str:
        ...
        try:
            ...
            # Datos normales
            normal_data = df[(df[main_var] >= lower_bound) & (df[main_var] <= upper_bound)]
            fig.add_trace(go.Scatter(
                x=normal_data.index,
    ...
    def create_summary_dashboard(self, df: pd.DataFrame, output_dir: str) -> str:
        ...
        try:
            ...
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=('Temperatura Promedio', 'Precipitación', 
                              'Humedad Relativa', 'Distribución Temporal'),
            ...
            if 'humedad_relativa' in df.columns:
                fig.add_trace(
                    go.Histogram(x=df['humedad_relativa'], name='Humedad'),
                    row=2, col=1
                )
            ...
            if 'fecha' in df.columns and 'temperatura_promedio' in df.columns:
                ...
                fig.add_trace(
                    go.Scatter(x=df_temp['fecha'], y=df_temp['temperatura_promedio'],
    ...
    def export_plots_to_png(self, html_files: Dict[str, str], output_dir: str) -> Dict[str, str]:
        """
