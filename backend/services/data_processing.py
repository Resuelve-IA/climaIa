...
class DataProcessingService:
    ...
    def _validate_temporal_consistency(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(
            ...
            invalid_mask = (df["temperatura_minima"] > df["temperatura_promedio"]) | (
        ...
        try:
            ...
            if "latitud" in processed_data and "longitud" in processed_data:
                ...
                if not self.geo_processor.is_in_cundinamarca(lat, lon):
                    logger.warning(
                        f"Estación {processed_data.get('codigo', 'N/A')} fuera de Cundinamarca"
                    )
    ...
    def calculate_statistics(
        self, df: pd.DataFrame, group_by: str = "estacion"
    ) -> Dict:
        """
        ...
        try:
            ...
            if group_by not in df.columns:
                ...
                for group_name, group_data in grouped:
                    stats[group_name] = {}
                    numeric_columns = group_data.select_dtypes(
                        include=[np.number]
                    ).columns
