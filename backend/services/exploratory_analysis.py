...
class ExploratoryAnalysisService:
    ...
    def _generate_data_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "stations_count": df["estacion"].nunique()
    ...
    def _analyze_temporal_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        analysis = {
            "temporal_coverage": {
                ...
                "date_range_days": (
                    df_temp["fecha"].max() - df_temp["fecha"].min()
                ).days,
            },
            "seasonal_patterns": {},
            "monthly_patterns": {},
        ...
        for col in numeric_columns:
            if col in df_temp.columns and col not in ["year", "month", "day_of_year"]:
                ...
                # Patrones anuales
                yearly_stats = (
                    df_temp.groupby("year")[col].agg(["mean", "std", "count"]).to_dict()
                )
    ...
    def _analyze_correlations(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(numeric_columns) < 2:
            return {
                "error": "Insuficientes variables numéricas para análisis de correlación"
            }
        ...
        for col in numeric_columns:
            if col in df_temp.columns and col not in ["month"]:
                ...
                if len(monthly_means) > 1:
                    trend = np.polyfit(
                        range(len(monthly_means)), monthly_means.values, 1
                    )[0]
                    analysis["seasonal_trends"][col] = {
                        "trend_slope": float(trend),
                        "trend_direction": "incremental"
                        if trend > 0
                        else "decremental",
                    }
    ...
    def _analyze_station_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in numeric_columns:
            if col in df.columns:
                ...
                analysis["station_comparison"][col] = {
                    ...
                    "station_with_highest_mean": max(
                        station_means, key=station_means.get
                    ),
                    "station_with_lowest_mean": min(
                        station_means, key=station_means.get
                    ),
                }
        ...
        for station in df["estacion"].unique():
            ...
            completeness = (
                station_data.notnull().sum().sum()
                / (len(station_data) * len(station_data.columns))
            ) * 100
    ...
    def _generate_data_quality_report(self, df: pd.DataFrame) -> Dict[str, Any]:
                ...
                if "temperatura" in col.lower():
                    if df[col].min() < -50 or df[col].max() > 60:
                        consistency_issues.append(
                            f"Valores de temperatura fuera de rango en {col}"
                        )
                elif "humedad" in col.lower():
                    if df[col].min() < 0 or df[col].max() > 100:
                        consistency_issues.append(
                            f"Valores de humedad fuera de rango en {col}"
                        )
        ...
        report["overall_quality_score"] = (
            completeness_score + report["consistency"]["consistency_score"]
        ) / 2

        return report

    def _generate_eda_visualizations(
    ...
    def _generate_recommendations(self, df: pd.DataFrame) -> List[str]:
        ...
        if missing_percentage > 20:
            recommendations.append(
                "Alto porcentaje de datos faltantes. Considerar estrategias de imputación."
            )

        # Verificar cobertura temporal
        if "fecha" in df.columns:
            date_range = pd.to_datetime(df["fecha"].max()) - pd.to_datetime(
                df["fecha"].min()
            )
            if date_range.days < 365:
                recommendations.append(
                    "Cobertura temporal limitada. Se recomienda extender el período de análisis."
                )
        ...
        if "estacion" in df.columns:
            ...
            if station_count < 5:
                recommendations.append(
                    "Pocas estaciones meteorológicas. Considerar incluir más estaciones para mejor cobertura espacial."
                )
        ...
        if len(numeric_columns) > 1:
            correlation_matrix = df[numeric_columns].corr()
            high_correlations = (correlation_matrix > 0.8).sum().sum() - len(
                correlation_matrix
            )
            if high_correlations > 0:
                recommendations.append(
                    "Se detectaron variables altamente correlacionadas. Considerar análisis de multicolinealidad."
                )

        if not recommendations:
            recommendations.append(
                "Los datos muestran buena calidad general. Continuar con análisis más avanzados."
            )
    ...
    def _save_analysis_results(
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- Rango de fechas: {results['summary']['date_range']['start']} a {results['summary']['date_range']['end']}\n"
            )
            f.write(f"- Número de estaciones: {results['summary']['stations_count']}\n")
            f.write(
                f"- Porcentaje de datos faltantes: {results['summary']['missing_data_percentage']:.2f}%\n\n"
            )

            # Calidad de datos
            f.write("CALIDAD DE DATOS:\n")
            f.write(
                f"- Score general de calidad: {results['quality_report']['overall_quality_score']:.2f}%\n"
            )
            f.write(
                f"- Completitud: {results['quality_report']['completeness']['overall_completeness']:.2f}%\n"
            )
            f.write(
                f"- Consistencia: {results['quality_report']['consistency']['consistency_score']:.2f}%\n\n"
            )
    ...
    def generate_station_report(
        self, station_code: str, df: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if len(station_data) == 0:
                raise ValueError(
                    f"No se encontraron datos para la estación {station_code}"
                )
            ...
            report = {
                "station_info": {
                    ...
                    "date_range": {
                        "start": station_data["fecha"].min().isoformat()
                        if "fecha" in station_data.columns
                        else None,
                        "end": station_data["fecha"].max().isoformat()
                        if "fecha" in station_data.columns
                        else None,
                    },
                },
            ...
            logger.error(
                f"Error generando reporte de estación {station_code}: {str(e)}"
            )
            raise
